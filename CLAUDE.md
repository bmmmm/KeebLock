...
2. **`scripts/build_dyk.py`** — heuristic generator for 5 "Did you know?"
   snippets per codeword. The shipped corpus was actually generated by Haiku
   sub-agents per theme for higher quality; this script is the offline
   fallback. Re-run if you change `facts` or want to regenerate without an
   API key.
...
- Source-of-truth remote is Forgejo at `git.home/bsz/KeebLock` (SSH on port
