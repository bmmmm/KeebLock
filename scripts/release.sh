...
- https://git.home/bsz/KeebLock  (origin, internal)
EOF
)"
fi
...
( cd "$HOME" && tea releases create "$TAG" \
      ...
      --note "$NOTES" )
( cd "$HOME" && tea releases assets create "$TAG" "$ZIP_ABS" --repo bsz/KeebLock )

echo
echo "==> Done."
echo "    Forgejo: http://git.home/bsz/KeebLock/releases/tag/$TAG"
